;(function ($) {
    ...
    let handleCollapseGiaoHang = function () {
        $('#collapse-multi .collapse').on('show.bs.collapse', function (e) {
            let collapseID = $(this).attr('id'),
                collapseItem = $('.input-collapse[data-bs-target="#' + collapseID + '"]').parent('.form-collapse_item');
            collapseItem.addClass('is-collapsed');
        }).on('hide.bs.collapse', function (e) {
            let collapseID = $(this).attr('id'),
                collapseItem = $('.input-collapse[data-bs-target="#' + collapseID + '"]').parent('.form-collapse_item');
            collapseItem.removeClass('is-collapsed');
        });

        $('.trigger-inputGiaoTanNoi').click(function () {
            $('[data-bs-target="#collapseGiaoTanNoi"]').click();
        });
    }
    ...
    let handleDropdownPromo = function () {
        $('.cart-promo_dropdown__button').click(function () {
            $(this).closest('.cart-promo').toggleClass('is-show');
        })
    }

    let handleInitSelect = function () {
        $('.initSelect').each(function (e) {
            let elm = '';
            ...
            $(this).select2({
                ...
                $(this).next().next('.select2-container').find('.select2-search__field').attr('placeholder', placeholderText);
            });
        });
    }
    ...
    let handleTouchMoveFloatingCart = function (ev) {
        if (!$(ev.target).closest('#floating-cart .floating-cart_inner').length) {
            ev.preventDefault();
        }
    }
    let handleFloatingCart = function () {
        let btnCall = $('.call-floatingCart'),
            btnClose = $('.close-floatingCart'),
            elmFloatingCart = $('#floating-cart'),
            elmFloatingCartOverlay = elmFloatingCart.find('.floating-cart_overlay'),
            elmBody = $('body'),
            elmRemoveCart = $('.floating-cart_remove'),
            elmPreloader = elmFloatingCart.find('#floating-cart_preloader'),
            elmCartEmpty = elmFloatingCart.find('#floating-cart_empty');

        btnCall.click(function () {
            if (elmFloatingCart.hasClass('is-show')) {
                elmBody.attr({
                    'style': ''
                });
                document.removeEventListener('touchmove', handleTouchMoveFloatingCart);
                elmFloatingCart.removeClass('is-show');
            } else {
                document.addEventListener('touchmove', handleTouchMoveFloatingCart, {passive: false});
                elmBody.attr({
                    'style': 'overflow-y: hidden'
                });
                elmFloatingCart.addClass('is-show');
            }
        });

        btnClose.add(elmFloatingCartOverlay).click(() => {
            elmBody.attr({
                'style': ''
            });
            document.removeEventListener('touchmove', handleTouchMoveFloatingCart);
        ...
        elmRemoveCart.click(function () {
            let elmThis = $(this),
                elmTable = elmThis.parents('.table-list_cart');
            elmPreloader.addClass('is-loading');
            setTimeout(function () {
                elmThis.parents('.cart-item').fadeOut().remove();
                elmPreloader.removeClass('is-loading');

                if (elmTable.find('.cart-item').length == 0) {
                    elmTable.fadeOut().remove();
                    $('.floating-cart_footer').fadeOut().remove();
                    elmCartEmpty.addClass('is-show');
                    console.log(2);
                }
            }, 1000);
        });
